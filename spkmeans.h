...
static double** jacobi(double** matrix, int n) {
    ...
    double **newMatrix, **rotationMatrix = identity_mat(n);
...
static int eigen_gap(int n) {
    int i, argmax = 0;
    double max = 0;
    double *eigens = (double *) malloc(n * sizeof(double));
    if (eigens == NULL)
        return -1;

    for(i = 0; i < n; i++)
        eigens[i] = eigen_mat[i][i];
    free_Mem(eigen_mat, n);

    qsort(eigens, n, sizeof(double), compare);
    n = n / 2;
    for (i = 0; i < n; i++){
        if (fabs(eigens[i] - eigens[i+1]) > max){
            max = fabs(eigens[i] - eigens[i+1]);
            argmax = i;
        }
    }
    return argmax+1;
